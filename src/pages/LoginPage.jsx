...
function Login() {
  ...
  if (profileError || !profile) {
    setError('Login successful, but user profile not found.');
    console.error('🔍 Profile fetch error:', profileError?.message || 'No profile data');
    setLoading(false);
    return;
  }

  // Check status and role
  const isActive = profile.status?.toLowerCase().trim() === 'active';
  const isRecruiter = profile.role === 'Recruiter';

  if (!isActive) {
    setError('Your account is not active. Please contact the admin.');
  } else if (!isRecruiter) {
    setError('Access denied. Only Recruiter accounts are allowed.');
  } else {
    console.log('🎉 Authorized Recruiter:', profile);
